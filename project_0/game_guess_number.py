''' Guess the number game. Computer takes a number and tries to guess it itself within 20 tries'''
...
def start_guess_counter(number:int): 
...
print(f'{guess_number(number)} - количество попыток компьютера угадать число')
...
print(f'Алгоритм угадывает число в среднем за {average_result(guess_number)} попыток/-ки')
